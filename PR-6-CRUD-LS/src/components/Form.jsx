...
const Form = ({ addUser, editUser, updateUser }) => {
    ...
    useEffect(() => {
        if (editUser) {
            ...
            setInput({ name: '', gender: '', phone: '', email: '', password: '', cpassword: '' });
        }
    }, [editUser])
    ...
    return (
        <div className='container mx-auto my-4 px-10'>
            ...
            <form onSubmit={handleSubmit} ref={formRef} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                        ...
                        <ul className="p-0 m-0 items-center text-sm font-medium text-gray-900 rounded-lg sm:flex">
                            <li className="w-full border-b border-gray-200 sm:border-b-0 sm:border-r">
                                <div className="flex items-center ps-3">
                                    <input id="male" checked={input.gender === "male"}
                                        name='gender' value='male' type="radio" onChange={(e) => {
                            ...
                            <li className="w-full border-b border-gray-200 sm:border-b-0 sm:border-r">
                                <div className="flex items-center ps-3">
                                    <input id="female" checked={input.gender === "female"}
                                        name='gender' value='female' type="radio" onChange={(e) => {
                                            setInput({ ...input, [e.target.name]: e.target.value })
                                            setErrors({ ...errors, [e.target.name]: '' })
                                        }} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500" />
                ...
                <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">{editUser ? "Update" : "Submit"}</button>
            </form>
