            ...
            <tbody>
                {
                    employees.map((employee, idx) => {
                        return <tr key={employee.id} className="bg-white dark:bg-gray-900 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                            ...
                            <td className="px-6 py-4">{Number(employee.salary).toLocaleString('en-IN')}</td>
                            <td className="px-6 py-4">{getDepartment(Number(employee.department))}</td>
                            <td className="px-6 py-4 flex justify-center gap-5">
                                <Link to={`/edit-employee/${employee.id}`} className="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-semibold transition">Edit</Link>
                                <button className="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 font-semibold transition" onClick={() => handleDelete(employee.id)}>Delete</button>
                            </td>
                        </tr>
                    })
                }
